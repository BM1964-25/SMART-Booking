...
const helpSections: HelpSection[] = [
  ...
  },
  {
    ...
    bullets: [
      ...
      "Apple Kalenderauswahl: Nach erfolgreicher Verbindung lädt SMART Booking die verfügbaren iCloud-Kalender. Die Kalender-ID muss nicht manuell gesucht werden; Sie wählen den passenden Kalender in der Liste aus.",
      "Google Kalender: Benötigt zuerst eine Google-OAuth-Verbindung. Dafür werden Google OAuth Client ID und Google OAuth Client Secret aus einem Google-Cloud-Projekt verwendet.",
      "Google OAuth Client ID und Secret: Diese Werte kommen aus der Google Cloud Console unter APIs & Dienste, Anmeldedaten, OAuth-Client. Danach wird das Google-Konto verbunden und die verfügbaren Kalender werden automatisch geladen.",
      "Google Buchungskalender: Der Kalender muss zum verbundenen Google-Konto passen. SMART Booking zeigt die Kalender nach der Verbindung an; Sie wählen einen Kalender für neue Buchungen und optional weitere für die Verfügbarkeitsprüfung.",
      "Microsoft 365 / Outlook: Benötigt eine Microsoft-App-Registrierung mit Microsoft Client ID und Microsoft Client Secret. Die Werte kommen aus dem Microsoft Entra Admin Center beziehungsweise Azure-Portal.",
      "Microsoft-Verbindung: Nach erfolgreicher Verbindung zeigt SMART Booking die Outlook-/Microsoft-Kalender des verbundenen Kontos an. Auch hier wird genau ein Buchungskalender gewählt und optional weitere Kalender für den Abgleich aktiviert.",
      ...
      "API aktiv: Diese Auswahl bedeutet, dass SMART Booking nicht nur einen festen Link wiederverwendet, sondern versucht, pro Buchung automatisch einen neuen Meeting-Link über den Anbieter zu erzeugen.",
      "Fester Link: Ein dauerhaft hinterlegter Meeting-Link, der bei jeder passenden Buchung übernommen wird. Diesen Link kopieren Sie direkt aus Zoom, Google Meet oder Microsoft Teams, zum Beispiel aus einem persönlichen Meetingraum oder einem manuell erstellten Dauertermin.",
      "Zoom fester Link: Nutzt bei jeder Zoom-Buchung denselben dauerhaft hinterlegten Zoom-Link. Geeignet ist zum Beispiel der persönliche Zoom-Raum oder ein bewusst wiederverwendeter Meeting-Link.",
      "Zoom API: Erzeugt pro Buchung automatisch einen eigenen Zoom-Link, wenn Account ID, Client ID und Client Secret vollständig hinterlegt und in Zoom aktiv sind.",
      "Zoom Account ID: Dieser Wert kommt aus der Zoom Server-to-Server-OAuth-App im Zoom App Marketplace. Es ist eine technische Konto-ID und kein Meeting-Link.",
      "Zoom Client ID: Dieser Wert kommt ebenfalls aus der Zoom Server-to-Server-OAuth-App. Er identifiziert die Zoom-App, mit der SMART Booking die API nutzt.",
      "Zoom Client Secret: Das geheime API-Passwort der Zoom Server-to-Server-OAuth-App. Es wird in Zoom erzeugt oder angezeigt und sollte vertraulich behandelt werden.",
      "Zoom API prüfen: Der Prüfbutton testet, ob die gespeicherten Zoom-API-Daten grundsätzlich akzeptiert werden. Wenn Zoom „invalid_client“ meldet, sind meistens Client ID, Client Secret, Account ID oder die Freigabe der Zoom-App falsch.",
      "Google Meet fester Link: Nutzt bei jeder Google-Meet-Buchung denselben dauerhaft hinterlegten Meet-Link. Diesen Link erhalten Sie in Google Meet oder aus einem Google-Kalendertermin, zum Beispiel https://meet.google.com/abc-defg-hij.",
      "Google Meet API: Erzeugt pro Buchung automatisch einen eigenen Google-Meet-Link, wenn Google verbunden ist und Google Kalender als aktiver Buchungskalender genutzt wird.",
      "Google-Verbindung: Die angezeigte Google-Adresse zeigt, welches Google-Konto verbunden ist. Der Buchungskalender muss zu dieser Verbindung passen, damit automatische Meet-Links erzeugt werden können.",
      "Microsoft Teams fester Link: Nutzt bei jeder Teams-Buchung denselben dauerhaft hinterlegten Teams-Link. Diesen Link kopieren Sie aus einem Teams-Meeting oder aus dem Microsoft-365-Kalendertermin.",
      "Microsoft Teams API: Funktioniert, sobald Microsoft 365 / Outlook verbunden und als aktiver Kalenderanbieter gespeichert ist. Dann kann die spätere automatische Teams-Erzeugung über Microsoft Graph genutzt werden.",
